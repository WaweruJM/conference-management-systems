...
//   • For each item in the session: a cover page (talk title + author photo + bio
//     + session time slot) followed by the presenter's uploaded PDF slides.
// Returns { pdfBytes: Uint8Array, slideIndex, totalPages }
export async function generateMergedConferencePDF({ conference, sessions = [], uploadDir = '/app/uploads' }) {
  ...
  // ---------- CONFERENCE COVER ----------
  {
    ...
    p.drawText(`Sessions: ${sessions.length}`, { x: 60, y: 120, size: 11, font: helvB, color: slate })
    p.drawText(`Presentations: ${totalTalks}`, { x: 60, y: 100, size: 11, font: helvB, color: slate })
  ...
  for (const s of sessions) {
    ...
    wrap(sp, s.title || 'Untitled Session', { x: 60, y: H / 2 + 20, size: 22, font: helvB, color: slate, maxWidth: W - 120, lineHeight: 26, maxLines: 3 })
    const timeLine = `${fmtDay(s.startTime)} · ${fmtTime(s.startTime)} – ${fmtTime(s.endTime)}`
    sp.drawText(timeLine, { x: 60, y: H / 2 - 60, size: 12, font: helv, color: muted })
    const meta = [s.room && `Room: ${s.room}`, s.chair && `Chair: ${s.chair}`, s.theme?.name && `Theme: ${s.theme.name}`].filter(Boolean).join(' · ')
    if (meta) sp.drawText(meta, { x: 60, y: H / 2 - 80, size: 11, font: helv, color: muted, maxWidth: W - 120 })
    ...
    for (const item of (s.items || [])) {
      ...
      cp.drawText(fmtDay(s.startTime), { x: W - 220, y: H - 40, size: 10, font: helv, color: white })
      cp.drawText(`${fmtTime(s.startTime)} – ${fmtTime(s.endTime)}`, { x: W - 220, y: H - 58, size: 9, font: helv, color: paleIndigo })
      ...
      cp.drawText(`Talk ${slideIndex.length + 1} of ${totalTalks}`, { x: W - 160, y: 15, size: 9, font: helvB, color: indigo })
