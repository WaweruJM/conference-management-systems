...
import { abbreviateRank, isKdfRank } from './kdf-ranks'

// ─────────────────────────────────────────────────────────────────────────────
// Internal helper — resolve a stored logo path to an absolute filesystem path
// so pdfkit can embed it. Returns null on any failure (missing file, wrong
// mime, etc.) so callers can fall back to a placeholder gracefully.
// ─────────────────────────────────────────────────────────────────────────────
async function _resolveLogoBuffer(logoPath) {
  if (!logoPath) return null
  try {
    // logoPath is stored as e.g. "/api/uploads/header/<confId>/foo.png"
    // Strip the "/api/uploads/" prefix and resolve to UPLOAD_DIR.
    const uploadDir = process.env.UPLOAD_DIR || '/app/uploads'
    const rel = logoPath.replace(/^\/?api\/uploads\/?/, '').replace(/^\/+/, '')
    const abs = path.join(uploadDir, rel)
    const buf = await fs.readFile(abs)
    // Only PNG / JPG are safe for pdfkit
...
// ─────────────────────────────────────────────────────────────────────────────
// Compose a delegate display name with KDF rank shorthand where applicable.
//   rank + fullName  →  "Maj John Doe"       (in-service)
//   civilian title   →  "Dr. Jane Doe"       (rank field left blank at reg)
// ─────────────────────────────────────────────────────────────────────────────
function _formatNameLine({ prefix, fullName, rank }) {
  const parts = []
  const rk = (rank || '').trim()
  if (isKdfRank(rk)) {
    // In-service: prepend abbreviated rank, drop civilian prefix
    parts.push(abbreviateRank(rk))
  } else if (prefix) {
    parts.push(prefix)
  }
...
export async function generateNameTagsPDF(delegates, conferenceInfo) {
  // Attempt to load the KDF logo once (reused across all tags)
  const kdfLogoBuf = await _resolveLogoBuffer(
    conferenceInfo.headerLogoRight || conferenceInfo.kdfLogoPath
  )
  ...
  return new Promise((resolve, reject) => {
    ...
    // Palette — subdued academic navy / gold instead of playful indigo
    const NAVY   = '#0f2a4f'
    const GOLD   = '#8a6d17'
    const CREAM  = '#faf7ef'
    const INK    = '#0f172a'
    ...
    delegates.forEach((d, i) => {
      ...
      // ── Outer navy frame + inner gold hairline (formal double border) ──
      doc.roundedRect(x + 4, y + 4, tagW - 8, tagH - 8, 6)
         .lineWidth(1.8).stroke(NAVY)
      doc.roundedRect(x + 8, y + 8, tagW - 16, tagH - 16, 4)
         .lineWidth(0.4).stroke(GOLD)

      // ── Top section: KDF logo (or placeholder square) ───────────────
      const logoSize = 34
      const logoX = x + (tagW - logoSize) / 2
      const logoY = y + 14
      if (kdfLogoBuf) {
        try {
          doc.image(kdfLogoBuf, logoX, logoY, { width: logoSize, height: logoSize })
        } catch {
          _drawPlaceholderLogo(doc, logoX, logoY, logoSize, NAVY, GOLD)
        }
      } else {
        _drawPlaceholderLogo(doc, logoX, logoY, logoSize, NAVY, GOLD)
      }
      ...
      const ruleY = y + 14 + logoSize + 6
      doc.moveTo(x + tagW * 0.20, ruleY).lineTo(x + tagW * 0.80, ruleY)
         .lineWidth(0.5).stroke(GOLD)

      // ── Middle: name line (rank shorthand + full name) ──────────────
      const nameLine = _formatNameLine(d)
      const affiliation = d.affiliation || ''
      const unit = d.unit || ''

      doc.fillColor(INK).font('Helvetica-Bold').fontSize(15)
         .text(nameLine || '—', x + 8, ruleY + 10, { width: tagW - 16, align: 'center' })

      // Unit / department line (small, muted)
      if (unit) {
        doc.font('Helvetica').fontSize(9).fillColor(MUTED)
           .text(unit, x + 8, ruleY + 32, { width: tagW - 16, align: 'center' })
      }

      // Affiliation
      if (affiliation) {
        doc.font('Helvetica-Oblique').fontSize(9).fillColor(MUTED)
           .text(affiliation, x + 8, ruleY + (unit ? 46 : 34), { width: tagW - 16, align: 'center' })
      }

      // ── Bottom: conference title band ────────────────────────────────
      const bandH = 22
      const bandY = y + tagH - bandH - 6
      doc.rect(x + 8, bandY, tagW - 16, bandH).fill(CREAM)
      doc.fillColor(NAVY).font('Helvetica-Bold').fontSize(7.5)
         .text((conferenceInfo.name || 'Scientific Conference').toUpperCase(),
               x + 10, bandY + 7, { width: tagW - 20, align: 'center' })
    })
...
// Fallback badge when no KDF logo asset has been uploaded yet — a simple
// gold-bordered square with the letters "KDF" so the layout stays intact.
function _drawPlaceholderLogo(doc, x, y, size, borderColor, textColor) {
  doc.rect(x, y, size, size).lineWidth(0.8).stroke(borderColor)
  doc.fillColor(textColor).font('Helvetica-Bold').fontSize(10)
     .text('KDF', x, y + size / 2 - 6, { width: size, align: 'center' })
}
...
export function generateCertificatePDF({ recipient, conference, kind }) {
  return new Promise((resolve, reject) => {
    ...
    // Recipient name (with KDF rank shorthand for in-service personnel)
    const rk = (recipient.rank || '').trim()
    const nameParts = []
    if (isKdfRank(rk)) {
      nameParts.push(abbreviateRank(rk))
    } else if (recipient.prefix) {
      nameParts.push(recipient.prefix)
    }
    nameParts.push(recipient.fullName || `${recipient.firstName || ''} ${recipient.lastName || ''}`.trim())
    ...
    // Rank / Affiliation — only render affiliation for in-service; civilians
    // already had their prefix incorporated above so we don't repeat rank here.
    if (recipient.affiliation) {
      doc.fillColor('#64748b').fontSize(12)
         .text(recipient.affiliation, 60, 340, { width: W - 120, align: 'center' })
    }
  ...
  for (const it of withTimes) {
    if (it.type === 'talk') {
      ...
      if (presPath && presPath.toLowerCase().endsWith('.pdf')) {
        try {
          const stat = await fs.stat(presPath)
          // SECURITY: reject presentation PDFs > 60 MB during merging to prevent
          // out-of-memory attacks via crafted files.
          if (stat.size > 60 * 1024 * 1024) throw new Error('presentation too large to merge')
          const bytes = await fs.readFile(presPath)
          const src = await PDFLibDocument.load(bytes, { ignoreEncryption: true })
          // SECURITY: cap source page count too — malicious PDFs with 10k+
          // pages could exhaust memory during copy.
          if (src.getPageCount() > 500) throw new Error('presentation has too many pages')
